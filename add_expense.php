...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Calculate amount per piece if quantity is valid
    $amount_per_piece = ($quantity !== 'N/A' && is_numeric($quantity) && $quantity > 0)
        ? round($total_amount / $quantity, 2)
        : null;
    ...
    if (
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO expenses (category_id, description, total_amount, quantity, amount_per_piece, payment_method, purchase_date, user_id) 
                                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $category_id,
                $description,
